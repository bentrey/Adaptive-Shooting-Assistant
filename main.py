...
#overwriting the print function to record the data from blue_st_sdk
...
# node has been discovered or that the scanning starts/stops. This code comes
# from ST
...
#This function accesses the speaker
...
#this function removes the excess and missing data points
def trim_data(w,a):
    w2 = w['wx']**2 + w['wy']**2 + w['wz']**2
    a2 = a['ax']**2 + a['ay']**2 + a['az']**2
    center_index = w2.idxmax()
    empty_frame = (pd.DataFrame([],columns=w.columns),\
    ...
    if w2.max()>2250000 and center_index-5>=0 and center_index+5<len(w2):
        ...
        while (sum(list(w2[start_index:start_index+5]))/5 >50 or (w['time']\
        [center_index]-w['time'][start_index])<200)and start_index>1:
            start_index -= 1
        while sum(list(w2[stop_index-5:stop_index]))/5 >50 and stop_index<\
        len(w2)-1:
            stop_index += 1
...
# This function numberically integrates the given values in a rotating
# reference frame to find the final velocity.
def get_v(w, a):
    ...
    y = [0, 1, 0]
    z = [0, 0, 1]
    Ry = [[np.cos(theta),0,-np.sin(theta)], [0,1,0], [np.sin(theta),1,\
    np.cos(theta)]]
...
# This function runs the data getting prompt
def main(argv):
    global data
    global shots
    global makes
